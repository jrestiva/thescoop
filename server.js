...
function createComment(url, request) {
    ...
    if (requestComment && requestComment.username && requestComment.username != 'nonexistent_user' &&  requestComment.body && requestComment.articleId && database.articles[requestComment.articleId]) {
...
function deleteComment(url, request) {
    const id = Number(url.split('/').filter(segment => segment)[1]);
    const savedComment = database.comments[id];
    const savedArticles = database.articles;
    const response = {};
    ...
    if (savedComment) {
       ...
       // console.log(database.articles);
        ...
        //console.log("B4: "+JSON.stringify(userCommentIds)+ "INDEX OF: "+userCommentIds.indexOf(id));
        userCommentIds.splice(userCommentIds.indexOf(id), 1);
        //console.log("AFTER: "+userCommentIds);

        //database.comments.splice(id,1);

        database.comments[id] = null;

        Object.keys(savedArticles).forEach(function eachKey(key) {
          //  console.log(savedArticles[key].commentIds);

            savedArticles[key].commentIds = userCommentIds;
        });
        ...
        /*savedArticles.forEach(article => {
            ...
            console.log('loop: '+article.commentId);
        });*/



       // console.log("B4: "+JSON.stringify(database.articles)+ "INDEX OF: ");
        //userCommentIds.splice(userCommentIds.indexOf(id), 1);
        //console.log("AFTER: "+JSON.stringify(userCommentIds));


       /* console.log("B4: "+JSON.stringify(database.comments));
        database.comments.splice(database.comments.indexOf(id), 1);
        console.log("AFTER: "+JSON.stringify(database.comments));*/
